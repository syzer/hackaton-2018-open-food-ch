...
update_expiry_table <- function(expiry_table, inventory, id, date_bought, change_reason){
  item = inventory[inventory$id == id & inventory$DateBought==date_bought,]
  if (change_reason =="spoiled"){
    expiry_table[expiry_table$id == item$id, "expiry"] = expiry_table[expiry_table$id == item$id, "expiry"]-1 
